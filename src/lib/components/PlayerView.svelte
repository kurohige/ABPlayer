<script lang="ts">
  import { audioStore, togglePlay, skip, setSpeed, seek, seekToChapter, goToTrack, toggleMute, setSleepTimer, setSleepEndOfChapter, cancelSleepTimer, setAbRepeatA, setAbRepeatB, clearAbRepeat } from "../stores/audioStore";
      ...
      {#if !isFileless}
        ...
        {#if !collapsed}
          ...
          <div class="pv-util-bar">
            ...
            <button class="pv-util-icon" onclick={() => (showEq = true)} title="Equalizer">
              ...
              {#if showSleepMenu}
                <div class="sleep-menu" role="menu">
                  {#if $audioStore.sleepTimerMode !== "off"}
                    <button class="sleep-item active" role="menuitem" onclick={() => { cancelSleepTimer(); showSleepMenu = false; }}>Cancel timer</button>
                  {/if}
                  <button class="sleep-item" role="menuitem" onclick={() => { setSleepTimer(15); showSleepMenu = false; }}>15 minutes</button>
                  <button class="sleep-item" role="menuitem" onclick={() => { setSleepTimer(30); showSleepMenu = false; }}>30 minutes</button>
                  <button class="sleep-item" role="menuitem" onclick={() => { setSleepTimer(45); showSleepMenu = false; }}>45 minutes</button>
                  <button class="sleep-item" role="menuitem" onclick={() => { setSleepTimer(60); showSleepMenu = false; }}>60 minutes</button>
                  <button class="sleep-item" role="menuitem" onclick={() => { setSleepEndOfChapter(); showSleepMenu = false; }}>End of chapter</button>
                </div>
              {/if}
            </div>
          </div>
...
<style>
  ...
  @media (max-width: 720px) {
    .pv-hero:not(.collapsed) .pv-util-bar {
      display: none;
    }
    .pv-hero:not(.collapsed) .pv-hero-divider {
      display: none;
    }
  }
</style>
