<script lang="ts">
  ...
  import type { BookMeta, UserBookData, Collection, Tier } from "../types";
  ...
  let { book, userData, collections, position, tier = null, selectionMode = false, selected = false, isPlaying = false, focused = false, onedit, onplay, onremove, ontoggleselect }: Props = $props();
  ...
  function handleContextMenu(e: MouseEvent) {
    ...
    closeMenu = close;
    // Defer so the contextmenu event's trailing click doesn't immediately dismiss it
    setTimeout(() => {
      if (closeMenu === close) window.addEventListener("click", close);
    }, 0);
  }
...
>
  <button class="card-main" onclick={() => selectionMode && ontoggleselect ? ontoggleselect(book) : null} ondblclick={() => !selectionMode && onplay(book)}>
    <div class="cover">
      ...
      {:else if isFileless}
        <div class="logged-pill">LOGGED</div>
      ...
      {/if}
      {#if tier}
        <div class="tier-overlay">
          <TierBadge label={tier.label} color={tier.color} size={1.6} />
        </div>
      {/if}
