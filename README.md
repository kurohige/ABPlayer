...
- **Playback** — MP3, M4A, M4B, MP4, OGG, FLAC, WAV. Chapter navigation, speed control (0.75x-3x), resume prompts, chapter-scoped seeking for M4B.
- **Scroll-morphing player** — horizontal hero with cover, title, waveform, transport, and utility bar. Hero collapses to a pinned 56 px strip when the tab body scrolls past 8 px. Tabs: Chapters / Details / Rating.
...
Rust handler
    ...
    ├── Sets CORS headers (Access-Control-Allow-Origin: https://tauri.localhost)
...
- **CORS origin restriction** — audiostream protocol locked to `https://tauri.localhost` (was wildcard `*`).
...
src/                              # Svelte 5 frontend
  ...
  lib/
    components/                   # UI components
      ...
      PlayerView.svelte           # Scroll-morphing player with tabs
